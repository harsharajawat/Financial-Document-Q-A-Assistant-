...
class OllamaLLM:
    def __init__(self, api_url="http://localhost:11434/api/generate", model_name="smollm2:1.7b"):
    ...
    def invoke(self, prompt: str) -> str:
        ...
        try:
            ...
            data = response.json()
            # Adjust based on actual response format; common keys might be 'text' or 'result'
            generated_text = data.get("text") or data.get("result") or ""
            return generated_text.strip()
        except requests.exceptions.HTTPError as http_err:
            return f"Ollama API returned HTTP error: {http_err}"
        except requests.exceptions.RequestException as req_err:
            return f"Ollama API request failed: {req_err}"
...
def build_rag_pipeline(raw_text_chunks):
    ...
    vectorstore = Chroma.from_documents(documents=docs, embedding=embedder, persist_directory="./chroma_db")
    ...
    # Instantiate Ollama LLM wrapper
    ollama_llm = OllamaLLM()

    # Format documents to string for prompt input
    def format_docs(docs):
        return "\n\n".join(doc.page_content for doc in docs)
    formatted = format_docs(docs)
    print("RAG context fed to LLM:\n", formatted)  # debug print
        ...
        try:
            ...
            if question:
                ...
                if is_empty_answer(answer):
                    try:
                        rag_result = rag_pipeline.invoke(question)
